...
import { API_CLASS, API_POST_LIST, API_POST_DOCUMENT_GET } from '../../../constant/api-path';
...
export function useClassDetailService() {
  const { get, getBlob } = useApiService();
  ...
  const getFile = async (id: number, name: string): Promise<File> => {
    const blob = await getBlob(`${API_POST_DOCUMENT_GET}/${id}/${name}`);
